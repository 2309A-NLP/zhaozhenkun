...
from api.map import router as map_router                      # 高德地图 MCP 对接（出行/住宿/餐饮）
...
js_ast = _r("js/assistant.js") # 健康助理（挂号+咨询+地图入口）
js_map = _r("js/map.js")       # 高德地图出行/住宿/餐饮
...
# 内联所有 JS 文件（按加载顺序）
for js in [js_a, js_v, js_m, js_r, js_ast, js_map]:
    if js: FULL += "<script>\n" + js + "\n</script>\n"
FULL += "</body>\n</html>"

# 统计信息
_log.info("单文件HTML: %d bytes (CSS:%d JS:%d)",
    len(FULL), len(css), len(js_a)+len(js_v)+len(js_m)+len(js_r)+len(js_ast)+len(js_map))
...
app.include_router(map_router)            # POST /api/map/search|nearby|directions|chat
...
def map_nav():
    ...
    map_html = _r("map_nav.html")
    if map_html:
        return HTMLResponse(content=map_html)
    return HTMLResponse(content="<h1>地图页面加载失败</h1>", status_code=500)
