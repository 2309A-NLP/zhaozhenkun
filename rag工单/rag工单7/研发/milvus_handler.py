"""
milvus_handler.py - RAG工单7 Milvus向量库操作模块
工单编号: 人工智能NLP-RAG-功能测试及评估
功能: 管理CCF年报向量的Milvus集合操作，
      包括创建集合、插入向量、搜索检索
"""
...
class MilvusManager:
    ...
    def _ensure_collection(self):
        ...
        schema = self._CollectionSchema(fields, description=f"{MILVUS_COLL}-CCF年报")
    ...
    def close(self):
        """关闭连接"""
        if self._conn:
            self._conn.disconnect("default")


def create_index_and_insert(vectors, texts, metas):
    """
    便捷函数：创建Milvus索引并插入数据
    参数:
        vectors: 向量列表
        texts: 文本列表
        metas: 元数据列表
