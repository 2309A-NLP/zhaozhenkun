# -*- coding: utf-8 -*-
"""
ADSD 在线服务 - BGE模型管理器模块
负责加载和管理BGE（BAAI General Embedding）系列模型，包括：
1. BGE-M3嵌入模型：将文本转换为语义向量（用于向量检索）
2. BGE-Reranker重排序模型：对检索结果进行重新排序，提升相关性
提供文本嵌入（embed）和结果重排序（rerank）功能
"""
...
from online.utils import reduce_dimension  # 从utils模块导入降维函数，用于向量维度适配
...
class BGMManager:
    ...
    def embed(self, text: str) -> List[float]:
        ...
        if bge_model:  # 如果BGE-M3模型已成功加载
            try:  # 尝试使用BGE模型进行编码
                ...
                return vector  # 返回向量
            except Exception as e:  # 捕获编码过程中的异常
    ...
    def rerank(self, query: str, results: List[Dict], top_k: int) -> List[Dict]:
        ...
        try:  # 尝试执行重排序
            ...
            for i, r in enumerate(results):  # 遍历每个结果
        ...
        except Exception as e:  # 捕获重排序过程中的异常
