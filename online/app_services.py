...
# 尝试导入BGE管理器相关模块
try:
    from online.bge_manager import BGMManager, load_bge_models
except Exception:
...
# 尝试导入混合检索器
try:
    from online.hybrid_retriever import HybridRetriever
except Exception:
    HybridRetriever = None  # 导入失败时设为None

# 尝试导入LLM客户端
try:
    from online.llm_client import LLMClient
except Exception:
    LLMClient = None  # 导入失败时设为None

# 导入应用所需的其他模块
from online.app_monitor import percentile  # 百分位数计算函数
from online.app_text import DEFAULT_QUERY, SANITIZED_AVATARS, load_avatar_documents, load_local_documents, repair_text  # 文本处理相关
from online.milvus_manager import MilvusManager  # Milvus向量数据库管理器
from online.mysql_manager import MySQLManager  # MySQL数据库管理器
from online.output_optimizer import LLMOutputOptimizer  # LLM输出优化器
from online.redis_manager import RedisManager  # Redis缓存管理器
from online.short_term_memory import ShortTermMemory  # 短期记忆管理
from online.user_manager import UserManager  # 用户管理器
from online.utils import generate_doc_id  # 文档ID生成工具
