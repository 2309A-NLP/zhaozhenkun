...
负责管理用户的注册、登录和个人设置
主要功能：
1. 用户注册：创建新用户并保存到本地JSON文件，记录注册日志到MySQL
2. 用户登录：验证用户身份，如果用户不存在则自动注册（简化登录流程）
3. 头像设置：用户可在不同角色（医生/心理师/营销员）之间切换头像
4. 数据持久化：用户数据存储到本地JSON文件，操作日志记录到MySQL
"""
import json  # 导入JSON模块，用于读写JSON格式的用户数据文件
import threading  # 导入线程模块，用于实现线程安全的文件操作
from datetime import datetime  # 导入datetime模块，用于记录用户创建时间
from typing import Dict  # 导入类型提示，用于函数参数和返回值的类型注解
from online.utils import normalize_username  # 从工具模块导入用户名规范化函数
