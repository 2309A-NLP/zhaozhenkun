...
"""
ADSD 在线服务 - 混合检索器模块
实现融合BM25关键词检索、TF-IDF向量检索和倒数排名融合（RRF）算法的混合检索系统
主要功能：
1. BM25检索：基于词频和逆文档频率的关键词匹配
2. TF-IDF检索：基于词频-逆文档频率的向量相似度匹配
3. RRF融合：将多种检索结果融合为统一的排序列表
"""
...
from online.utils import tokenize_text, generate_doc_id  # 从自定义工具模块中导入分词函数和文档ID生成函数

try:  # 尝试导入BM25算法库
    from rank_bm25 import BM25Okapi  # 尝试导入BM25算法实现（用于关键词检索）
except Exception:  # 导入失败时处理
